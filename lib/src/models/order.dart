...
class Order {
  ...
  Order.fromJSON(Map<String, dynamic> jsonMap) {
    try {
      ...
      payment = jsonMap['payment'] != null ? Payment.fromJSON(jsonMap['payment']) : Payment.init();
      deliveryAddress = jsonMap['delivery_address'] != null ? Address.fromJSON(jsonMap['delivery_address']) : Address();
      foodOrders = jsonMap['food_orders'] != null ? List.from(jsonMap['food_orders']).map((element) => FoodOrder.fromJSON(element)).toList() : <FoodOrder>[];
    } catch (e) {
      id = '';
      tax = 0.0;
      deliveryFee = 0.0;
