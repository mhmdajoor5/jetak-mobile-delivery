...
Future<Object> acceptOrderWithId(int orderId) async {
  User _user = userRepo.currentUser.value;
  if (_user.apiToken == null) {
    return new Stream.value(new OrderStatus());
  }
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}${_user.id}/accept_order?$_apiToken';

  final client = new http.Client();
  final response = await await client.put(
    Uri.parse(url),
